...
ENV PATH /snap/bin:$PATH
    ...
    # && apt-get upgrade -y --allow-releaseinfo-change \
RUN apt-get update --allow-releaseinfo-change \
    && apt-get install -y --no-install-recommends \
       ca-certificates bash sudo git net-tools tcpdump maven gradle \
    && apt-get install -y --no-install-recommends \
       snapd fuse postgresql-client default-mysql-client mongo-tools \
    && apt-get install -y --no-install-recommends \
